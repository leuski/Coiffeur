...
class ALDocumentController : NSDocumentController {
  ...
  override func typeForContentsOfURL(url: NSURL, error outError: NSErrorPointer) -> String?
  {
  ...
  func openUntitledDocumentOfType(type:String, display displayDocument:Bool, error outError: NSErrorPointer) -> AnyObject?
  {
    let result: AnyObject? = self.makeUntitledDocumentOfType(type, error:outError)
