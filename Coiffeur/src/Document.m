...
- (instancetype)init {
    self = [super init];
    if (self) {
			NSString* options = [self runUncrustify:@[ @"--show-config" ] text:nil];
			if (![self readOptionsFromString:options])
				return self = nil;

			NSString* values = [self runUncrustify:@[ @"--update-config" ] text:nil];
			if (![self readValuesFromString:values])
				return self = nil;
...
- (BOOL)writeToURL:(NSURL *)absoluteURL ofType:(NSString *)typeName forSaveOperation:(NSSaveOperationType)saveOperation originalContentsURL:(NSURL *)absoluteOriginalContentsURL error:(NSError **)error
{
	...
	return [self writeValuesToURL:absoluteURL error:error];
}
...
- (IBAction)uncrustify:(id)sender
{
	...
	BOOL result = [self runUncrustify:@[ @"-q", @"-c", configPath ] text:text completionBlock:^(NSString* text){
...
{
	NSURL* executableURL = [[NSBundle mainBundle] URLForAuxiliaryExecutable:@"uncrustify"];
