...
- (void)setLabels:(NSArray*)labels
{
  ...
  NSAttributedString* attributedString
    = [[NSAttributedString alloc] initWithString:AL_SufficientlyLongWord attributes:attributes];
...
- (instancetype)initWithModel:(ALCoiffeurController*)model bundle:(NSBundle*)bundle;
{
  if (self = [super initWithNibName:NSStringFromClass([ALCoiffeurView class]) bundle:bundle]) {
    self.model = model;
    self.optionsSortDescriptors = @[[NSSortDescriptor sortDescriptorWithKey:ALNodeTitleKey
                                                                  ascending:YES
...
#pragma ide diagnostic ignored "OCNotLocalizedStringInspection"
  [container addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[childView]|"
                                                                    ...
                                                                    metrics:nil
                                                                      views:
                             NSDictionaryOfVariableBindings(childView)]];
...
- (IBAction)jumpToSection:(id)sender
{
  ...
  [self.optionsView
   scrollRowToVisible:[self.optionsView
                       rowForItem:popup.selectedItem.
                       representedObject]];
}
...
- (NSTableRowView*)outlineView:(NSOutlineView*)outlineView rowViewForItem:(id)item
{
  ...
  CGFloat smallSystemFontSize = [NSFont systemFontSizeForControlSize:NSSmallControlSize];
...
#pragma ide diagnostic ignored "OCNotLocalizedStringInspection"
  ...
  NSString*     hFormat = [NSString stringWithFormat:@"H:|-%d-[childView]|", offset];
